...
### gene-miRNA-candidates (output of SPONGE or SPONGEdb)
```{r, eval=FALSE}
...
## (G) 1. Train with random forests classifier
```{r, warning=FALSE, message=FALSE}
trained.model = calibrate_model(modules = train.modules, modules_metadata = train_cancer_metadata, label = "Subtype", sampleIDs = "PatientIDs",Metric = "Exact_match", n_folds = 1, repetitions = 1)
```

## (J) 2. Define random modules and calcualte spongEffects scores
```{r}
# Define random modules
Random.modules <- Random_spongEffects(sponge_modules = Sponge.modules,
                                      gene_expr = train.modules, min.size = 10,bin.size = 100,
                                      max.size = 200,min.expression=10, replace = F,
                                      method = "OE",cores = 1)
```

# Train classification model on randomly defined modules
```{r, warning=FALSE, message=FALSE}
Random.model = calibrate_model(modules = Random.modules, modules_metadata = train_cancer_metadata, label = "Subtype", sampleIDs = "PatientIDs",Metric = "Exact_match", n_folds = 1, repetitions = 1)
```
