...
```{r, warning=FALSE, message=FALSE}
trained.model = train_and_test_model(train_modules = train.modules, train_modules_metadata = train_cancer_metadata, test_modules = test.modules, test_modules_metadata = test_cancer_metadata, n_folds = 1, repititions = 1)
```
## (G) 2. Train and test on Central Genes
```{r, warning=FALSE, message=FALSE}
central.genes.model = build_classifier_central_genes(train_gene_expr = train_cancer_gene_expr, test_gene_expr = test_cancer_gene_expr, train_enrichment_modules = train.modules, test_enrichment_modules = test.modules, train_meta_data = train_cancer_metadata, test_meta_data = test_cancer_metadata, metric="Exact_match", n.folds=1)
```
