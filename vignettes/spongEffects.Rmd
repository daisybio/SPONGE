...
# Test Fabio - Markus 29.03.2022
```{r}
 # Define random modules
Random.Modules <- list()
for(j in 1:length(Sponge.modules)) {
    Module.Elements <-  sample.int(n = nrow(train_cancer_gene_expr), size = Size.modules[j],
                                       replace = F)
    # print(Module.Elements)
    Random.Modules[[j]] <- rownames(train_cancer_gene_expr)[Module.Elements]
    }
names(Random.Modules) <- names(Sponge.modules)
```

```{r}
Random <- enrichment_modules(gene_expr = train_cancer_gene_expr, modules = Random.Modules, bin.size = 10, min.size = 1, max.size = 2000, min.expr = 1, method = "OE")
```

# Original function 
```{r, warning=FALSE, message=FALSE}
random.model = build_classifier_random(sponge_modules= Sponge.modules,train_gene_expr = train_cancer_gene_expr, test_gene_expr = test_cancer_gene_expr, train_meta_data = train_cancer_metadata, test_meta_data = test_cancer_metadata, metric="Exact_match", n.folds=1, repetitions = 1, bin.size = 10, min.size = 1, max.size = 2000, method = "OE", min.expression =  1, replace = T)
```
