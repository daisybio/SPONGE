...
create_target_scan_symbols <- function(miranda) {
  ...
  # MERGE DATA
  merged.targets <- NULL
  targets_data <- list(miranda_targets, tarpmir_targets, pita_targets)
...
```{r, eval=FALSE}
miranda.bp <- data.frame(read.table("../bind_sites_processed.txt", header = T, sep = "\t"))
targetscan <- as.data.frame.matrix(table(miranda.bp$Target, miranda.bp$miRNA))
head(gene_expr)
```
...
num.of.cores <- 16 #many more on a compute cluster
